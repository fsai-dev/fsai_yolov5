...
def train(opt, device, callbacks=Callbacks()):
    ...
    # Logger
    logger = (
        GenericLogger(opt=opt, console_logger=LOGGER, include=("comet"))
        if RANK in {-1, 0}
        else None
    )
    ...
    if RANK in {-1, 0}:
        ...
        model.transforms = (
            val_loader.dataset.torch_transforms
    ...
    if RANK in {-1, 0} and final_epoch:
        ...
        # Plot examples
        images, labels = (
            x[:25] for x in next(iter(val_loader))
